...
export async function POST(request: Request) {
  console.log("Endpoint called - bybit/pairtradebuyalert");
  ...
  try {
    ...
    return new Response(
      JSON.stringify({
        message:
          "Endpoint success - bybit/pairtradebuyalert - buy order submitted",
      }),
      {
        status: 200,
        headers: {
    ...
    Sentry.captureException(error);
    console.error(
      "Endpoint error - bybit/pairtradebuyalert, error processing trade order:",
      error,
    );
